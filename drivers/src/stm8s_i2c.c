 ...
 * @version V2.3.0
 * @date    16-June-2017
 * @brief   This file contains all the functions for the T_I2C peripheral.
 ******************************************************************************
 * @attention
 *
...
/**
 * @brief  Deinitializes the T_I2C peripheral registers to their default reset values.
 * @param  None
 * @retval None
 */
void I2C_DeInit(void)
{
  T_I2C->CR1 = I2C_CR1_RESET_VALUE;
  T_I2C->CR2 = I2C_CR2_RESET_VALUE;
  T_I2C->FREQR = I2C_FREQR_RESET_VALUE;
  T_I2C->OARL = I2C_OARL_RESET_VALUE;
  T_I2C->OARH = I2C_OARH_RESET_VALUE;
  T_I2C->ITR = I2C_ITR_RESET_VALUE;
  T_I2C->CCRL = I2C_CCRL_RESET_VALUE;
  T_I2C->CCRH = I2C_CCRH_RESET_VALUE;
  T_I2C->TRISER = I2C_TRISER_RESET_VALUE;
}

/**
 * @brief  Initializes the T_I2C according to the specified parameters in standard
 *         or fast mode.
              ...
              I2C_AddMode_TypeDef AddMode, uint8_t InputClockFrequencyMHz)
{
  ...
  /*------------------------- T_I2C FREQ Configuration ------------------------*/
  /* Clear frequency bits */
  T_I2C->FREQR &= (uint8_t)(~I2C_FREQR_FREQ);
  /* Write new value */
  T_I2C->FREQR |= InputClockFrequencyMHz;

  /*--------------------------- T_I2C CCR Configuration ------------------------*/
  /* Disable T_I2C to configure TRISER */
  T_I2C->CR1 &= (uint8_t)(~I2C_CR1_PE);

  /* Clear CCRH & CCRL */
  T_I2C->CCRH &= (uint8_t)(~(I2C_CCRH_FS | I2C_CCRH_DUTY | I2C_CCRH_CCR));
  T_I2C->CCRL &= (uint8_t)(~I2C_CCRL_CCR);
  ...
  else /* STANDARD MODE */
  {
    ...
    T_I2C->TRISER = (uint8_t)(InputClockFrequencyMHz + (uint8_t)1);
  }

  /* Write CCR with new calculated value */
  T_I2C->CCRL = (uint8_t)result;
  T_I2C->CCRH = (uint8_t)((uint8_t)((uint8_t)(result >> 8) & I2C_CCRH_CCR) | tmpccrh);

  /* Enable T_I2C */
  T_I2C->CR1 |= I2C_CR1_PE;

  /* Configure T_I2C acknowledgement */
  I2C_AcknowledgeConfig(Ack);

  /*--------------------------- T_I2C OAR Configuration ------------------------*/
  T_I2C->OARL = (uint8_t)(OwnAddress);
  T_I2C->OARH = (uint8_t)((uint8_t)(AddMode | I2C_OARH_ADDCONF) |
                          (uint8_t)((OwnAddress & (uint16_t)0x0300) >> (uint8_t)7));
}

/**
 * @brief  Enables or disables the T_I2C peripheral.
 * @param  NewState : Indicate the new T_I2C peripheral state.
...
void I2C_Cmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* Enable T_I2C peripheral */
    T_I2C->CR1 |= I2C_CR1_PE;
  }
  else /* NewState == DISABLE */
  {
    /* Disable T_I2C peripheral */
    T_I2C->CR1 &= (uint8_t)(~I2C_CR1_PE);
  }
}
...
/**
 * @brief  Enables or disables the T_I2C General Call feature.
...
void I2C_GeneralCallCmd(FunctionalState NewState)
{
  ...
  {
    /* Enable General Call */
    T_I2C->CR1 |= I2C_CR1_ENGC;
  }
  else /* NewState == DISABLE */
  {
    /* Disable General Call */
    T_I2C->CR1 &= (uint8_t)(~I2C_CR1_ENGC);
  }
}

/**
 * @brief  Generates T_I2C communication START condition.
...
void I2C_GenerateSTART(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_I2C->CR2 |= I2C_CR2_START;
  }
  else /* NewState == DISABLE */
  {
    /* Disable the START condition generation */
    T_I2C->CR2 &= (uint8_t)(~I2C_CR2_START);
  }
}

/**
 * @brief  Generates T_I2C communication STOP condition.
...
/**
 * @brief  Enables or disables T_I2C software reset.
 * @param  NewState : Specifies the new state of the T_I2C software reset.
 *         This parameter can be any of the @ref FunctionalState enumeration.
 * @retval None
 */
void I2C_SoftwareResetCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_I2C->CR2 |= I2C_CR2_SWRST;
  }
  else /* NewState == DISABLE */
  {
    /* Peripheral not under reset */
    T_I2C->CR2 &= (uint8_t)(~I2C_CR2_SWRST);
  }
}

/**
 * @brief  Enables or disables the T_I2C clock stretching.
 * @param  NewState : Specifies the new state of the T_I2C Clock stretching.
 *         This parameter can be any of the @ref FunctionalState enumeration.
 * @retval None
 */
...
void I2C_StretchClockCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_I2C->CR1 &= (uint8_t)(~I2C_CR1_NOSTRETCH);
  }
  else /* NewState == DISABLE */
  {
    /* Clock Stretching Disable (Slave mode) */
    T_I2C->CR1 |= I2C_CR1_NOSTRETCH;
  }
}

/**
 * @brief  Enable or Disable the T_I2C acknowledge and position acknowledge feature.
...
void I2C_AcknowledgeConfig(I2C_Ack_TypeDef Ack)
{
  ...
  if (Ack == I2C_ACK_NONE)
  {
    ...
    T_I2C->CR2 &= (uint8_t)(~I2C_CR2_ACK);
  }
  else
  {
    /* Enable the acknowledgement */
    T_I2C->CR2 |= I2C_CR2_ACK;

    if (Ack == I2C_ACK_CURR)
    {
      /* Configure (N)ACK on current byte */
      T_I2C->CR2 &= (uint8_t)(~I2C_CR2_POS);
    }
    else
    {
      /* Configure (N)ACK on next byte */
      T_I2C->CR2 |= I2C_CR2_POS;
    }
  }
}

/**
 * @brief  Enables or disables the specified T_I2C interrupt.
...
void I2C_ITConfig(I2C_IT_TypeDef I2C_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* Enable the selected T_I2C interrupts */
    T_I2C->ITR |= (uint8_t)I2C_IT;
  }
  else /* NewState == DISABLE */
  {
    /* Disable the selected T_I2C interrupts */
    T_I2C->ITR &= (uint8_t)(~(uint8_t)I2C_IT);
  }
}

/**
 * @brief  Selects the specified T_I2C fast mode duty cycle.
...
void I2C_FastModeDutyCycleConfig(I2C_DutyCycle_TypeDef I2C_DutyCycle)
{
  ...
  {
    /* T_I2C fast mode Tlow/Thigh = 16/9 */
    T_I2C->CCRH |= I2C_CCRH_DUTY;
  }
  else /* I2C_DUTYCYCLE_2 */
  {
    /* T_I2C fast mode Tlow/Thigh = 2 */
    T_I2C->CCRH &= (uint8_t)(~I2C_CCRH_DUTY);
  }
}
...
uint8_t I2C_ReceiveData(void)
{
  ...
  return ((uint8_t)T_I2C->DR);
}

/**
 * @brief  Transmits the 7-bit address (to select the) slave device.
 * @param   Address : Specifies the slave address which will be transmitted.
 * @param   Direction : Specifies whether the T_I2C device will be a Transmitter or a Receiver.
...
void I2C_Send7bitAddress(uint8_t Address, I2C_Direction_TypeDef Direction)
{
  ...
  T_I2C->DR = (uint8_t)(Address | (uint8_t)Direction);
}
 ...
 ****************************************************************************************
 *
 *                         T_I2C State Monitoring Functions
 *
 ****************************************************************************************
 * This T_I2C driver provides three different ways for T_I2C state monitoring
 ...
 *          - I2C_IRQHandler() which is called when the T_I2C interrupts occur.
 ...
 *        - It is suitable when only one flag checking is needed (most T_I2C events
...
/**
 * @brief  Checks whether the last T_I2C Event is equal to the one passed
 *   as parameter.
...
ErrorStatus I2C_CheckEvent(I2C_Event_TypeDef I2C_Event)
{
  ...
  {
    lastevent = T_I2C->SR2 & I2C_SR2_AF;
  }
  else
  {
    flag1 = T_I2C->SR1;
 ...
 */
/**
 * @brief  Returns the last T_I2C Event.
 *
 * @note: For detailed description of Events, please refer to section
 *    I2C_Events in stm8s_i2c.h file.
 *
...
I2C_Event_TypeDef I2C_GetLastEvent(void)
{
  ...
  if ((T_I2C->SR2 & I2C_SR2_AF) != 0x00)
  {
    lastevent = I2C_EVENT_SLAVE_ACK_FAILURE;
  }
  else
  {
    /* Read the T_I2C status register */
    flag1 = T_I2C->SR1;
    flag2 = T_I2C->SR3;

    /* Get the last event value from T_I2C status register */
 ...
 */
/**
 * @brief  Checks whether the specified T_I2C flag is set or not.
...
FlagStatus I2C_GetFlagStatus(I2C_Flag_TypeDef I2C_Flag)
{
  ...
  /* Check the status of the specified T_I2C flag */
 ...
 *                         I2C_SR3 register ((void)(T_I2C->SR3)).
...
/**
 * @brief  Checks whether the specified T_I2C interrupt has occurred or not.
...
ITStatus I2C_GetITStatus(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
{
  ...
  enablestatus = (uint8_t)(T_I2C->ITR & (uint8_t)tempregister);

  if ((uint16_t)((uint16_t)I2C_ITPendingBit & REGISTER_Mask) == REGISTER_SR1_Index)
  {
    /* Check the status of the specified T_I2C flag */
    if (((T_I2C->SR1 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
    {
      /* I2C_IT is set */
      bitstatus = SET;
    }
  ...
  {
    /* Check the status of the specified T_I2C flag */
    if (((T_I2C->SR2 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
    {
      /* I2C_IT is set */
      bitstatus = SET;
    }
 ...
 *                    the T_I2C peripheral Acknowledge).
 *                  - ADD10 (10-bit header sent) is cleared by software
 *                    sequence: a read operation to I2C_SR1
