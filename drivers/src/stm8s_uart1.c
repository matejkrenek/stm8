 ...
 * @version V2.3.0
 * @date    16-June-2017
 * @brief   This file contains all the functions for the T_UART1 peripheral.
 ******************************************************************************
 * @attention
 *
...
UART1_DeInit(void)
{
  ...
  T_UART1->BRR2 = UART1_BRR2_RESET_VALUE; /* Set UART1_BRR2 to reset value 0x00 */
  T_UART1->BRR1 = UART1_BRR1_RESET_VALUE; /* Set UART1_BRR1 to reset value 0x00 */

  T_UART1->CR1 = UART1_CR1_RESET_VALUE; /* Set UART1_CR1 to reset value 0x00 */
  T_UART1->CR2 = UART1_CR2_RESET_VALUE; /* Set UART1_CR2 to reset value 0x00 */
  T_UART1->CR3 = UART1_CR3_RESET_VALUE; /* Set UART1_CR3 to reset value 0x00 */
  T_UART1->CR4 = UART1_CR4_RESET_VALUE; /* Set UART1_CR4 to reset value 0x00 */
  T_UART1->CR5 = UART1_CR5_RESET_VALUE; /* Set UART1_CR5 to reset value 0x00 */

  T_UART1->GTR = UART1_GTR_RESET_VALUE;
  T_UART1->PSCR = UART1_PSCR_RESET_VALUE;
}

/**
 * @brief  Initializes the T_UART1 according to the specified parameters.
                ...
                UART1_SyncMode_TypeDef SyncMode, UART1_Mode_TypeDef Mode)
{
  ...
  T_UART1->CR1 &= (uint8_t)(~UART1_CR1_M);

  /* Set the word length bit according to UART1_WordLength value */
  T_UART1->CR1 |= (uint8_t)WordLength;

  /* Clear the STOP bits */
  T_UART1->CR3 &= (uint8_t)(~UART1_CR3_STOP);
  /* Set the STOP bits number according to UART1_StopBits value  */
  T_UART1->CR3 |= (uint8_t)StopBits;

  /* Clear the Parity Control bit */
  T_UART1->CR1 &= (uint8_t)(~(UART1_CR1_PCEN | UART1_CR1_PS));
  /* Set the Parity Control bit to UART1_Parity value */
  T_UART1->CR1 |= (uint8_t)Parity;

  /* Clear the LSB mantissa of UART1DIV  */
  T_UART1->BRR1 &= (uint8_t)(~UART1_BRR1_DIVM);
  /* Clear the MSB mantissa of UART1DIV  */
  T_UART1->BRR2 &= (uint8_t)(~UART1_BRR2_DIVM);
  /* Clear the Fraction bits of UART1DIV */
  T_UART1->BRR2 &= (uint8_t)(~UART1_BRR2_DIVF);

  /* Set the T_UART1 BaudRates in BRR1 and BRR2 registers according to UART1_BaudRate value */
  BaudRate_Mantissa = ((uint32_t)CLK_GetClockFreq() / (BaudRate << 4));
  BaudRate_Mantissa100 = (((uint32_t)CLK_GetClockFreq() * 100) / (BaudRate << 4));
  /* Set the fraction of UART1DIV  */
  T_UART1->BRR2 |= (uint8_t)((uint8_t)(((BaudRate_Mantissa100 - (BaudRate_Mantissa * 100)) << 4) / 100) & (uint8_t)0x0F);
  /* Set the MSB mantissa of UART1DIV  */
  T_UART1->BRR2 |= (uint8_t)((BaudRate_Mantissa >> 4) & (uint8_t)0xF0);
  /* Set the LSB mantissa of UART1DIV  */
  T_UART1->BRR1 |= (uint8_t)BaudRate_Mantissa;

  /* Disable the Transmitter and Receiver before setting the LBCL, CPOL and CPHA bits */
  T_UART1->CR2 &= (uint8_t) ~(UART1_CR2_TEN | UART1_CR2_REN);
  /* Clear the Clock Polarity, lock Phase, Last Bit Clock pulse */
  T_UART1->CR3 &= (uint8_t) ~(UART1_CR3_CPOL | UART1_CR3_CPHA | UART1_CR3_LBCL);
  /* Set the Clock Polarity, lock Phase, Last Bit Clock pulse */
  T_UART1->CR3 |= (uint8_t)((uint8_t)SyncMode & (uint8_t)(UART1_CR3_CPOL |
                                                          UART1_CR3_CPHA | UART1_CR3_LBCL));

  if ((uint8_t)(Mode & UART1_MODE_TX_ENABLE))
  {
    /* Set the Transmitter Enable bit */
    T_UART1->CR2 |= (uint8_t)UART1_CR2_TEN;
  }
  else
  {
  ...
  if ((uint8_t)(Mode & UART1_MODE_RX_ENABLE))
  {
    ...
    T_UART1->CR2 |= (uint8_t)UART1_CR2_REN;
  }
  else
  {
  ...
  if ((uint8_t)(SyncMode & UART1_SYNCMODE_CLOCK_DISABLE))
  {
    ...
    T_UART1->CR3 &= (uint8_t)(~UART1_CR3_CKEN);
  }
  else
  {
    T_UART1->CR3 |= (uint8_t)((uint8_t)SyncMode & UART1_CR3_CKEN);
  }
}

/**
 * @brief  Enable the T_UART1 peripheral.
...
void UART1_Cmd(FunctionalState NewState)
{
  ...
  {
    /* T_UART1 Enable */
    T_UART1->CR1 &= (uint8_t)(~UART1_CR1_UARTD);
  }
  else
  {
    /* UART Disable */
    T_UART1->CR1 |= UART1_CR1_UARTD;
  }
}
...
void UART1_ITConfig(UART1_IT_TypeDef UART1_IT, FunctionalState NewState)
{
  ...
  /* Get the T_UART1 register index */
  uartreg = (uint8_t)((uint16_t)UART1_IT >> 0x08);
  /* Get the T_UART1 IT index */
  ...
  else
  {
    ...
    if (uartreg == 0x01)
    {
      T_UART1->CR1 &= (uint8_t)(~itpos);
    }
    else if (uartreg == 0x02)
    {
      T_UART1->CR2 &= (uint8_t)(~itpos);
    }
    else
    {
      T_UART1->CR4 &= (uint8_t)(~itpos);
    }
  }
}
...
void UART1_HalfDuplexCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    T_UART1->CR5 |= UART1_CR5_HDSEL; /**< T_UART1 Half Duplex Enable  */
  }
  else
  {
    T_UART1->CR5 &= (uint8_t)~UART1_CR5_HDSEL; /**< T_UART1 Half Duplex Disable */
  }
}
...
void UART1_IrDAConfig(UART1_IrDAMode_TypeDef UART1_IrDAMode)
{
  ...
  {
    T_UART1->CR5 |= UART1_CR5_IRLP;
  }
  else
  {
    T_UART1->CR5 &= ((uint8_t)~UART1_CR5_IRLP);
  }
}
...
void UART1_IrDACmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_UART1->CR5 |= UART1_CR5_IREN;
  }
  else
  {
    /* Disable the IrDA mode by clearing the IREN bit in the CR3 register */
    T_UART1->CR5 &= ((uint8_t)~UART1_CR5_IREN);
  }
}

/**
 * @brief  Sets the T_UART1 LIN Break detection length.
...
void UART1_LINBreakDetectionConfig(UART1_LINBreakDetectionLength_TypeDef UART1_LINBreakDetectionLength)
{
  ...
  {
    T_UART1->CR4 |= UART1_CR4_LBDL;
  }
  else
  {
    T_UART1->CR4 &= ((uint8_t)~UART1_CR4_LBDL);
  }
}

/**
 * @brief  Enables or disables the UART1�s LIN mode.
 * @param  NewState is new state of the T_UART1 LIN mode.
 *         This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
void UART1_LINCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_UART1->CR3 |= UART1_CR3_LINEN;
  }
  else
  {
    /* Disable the LIN mode by clearing the LINE bit in the CR2 register */
    T_UART1->CR3 &= ((uint8_t)~UART1_CR3_LINEN);
  }
}

/**
 * @brief  Enables or disables the T_UART1 Smart Card mode.
...
void UART1_SmartCardCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_UART1->CR5 |= UART1_CR5_SCEN;
  }
  else
  {
    /* Disable the SC mode by clearing the SCEN bit in the CR5 register */
    T_UART1->CR5 &= ((uint8_t)(~UART1_CR5_SCEN));
  }
}

/**
 * @brief  Enables or disables NACK transmission.
 * @note   This function is valid only for T_UART1 because is related to SmartCard mode.
 * @param  NewState: new state of the Smart Card mode.
 *         This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
void UART1_SmartCardNACKCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    T_UART1->CR5 |= UART1_CR5_NACK;
  }
  else
  {
    /* Disable the NACK transmission by clearing the NACK bit in the CR5 register */
    T_UART1->CR5 &= ((uint8_t) ~(UART1_CR5_NACK));
  }
}

/**
 * @brief  Selects the T_UART1 WakeUp method.
 * @param  UART1_WakeUp: specifies the T_UART1 wakeup method.
...
void UART1_WakeUpConfig(UART1_WakeUp_TypeDef UART1_WakeUp)
{
  ...
  T_UART1->CR1 &= ((uint8_t)~UART1_CR1_WAKE);
  T_UART1->CR1 |= (uint8_t)UART1_WakeUp;
}

/**
 * @brief  Determines if the T_UART1 is in mute mode or not.
 * @param  NewState: new state of the T_UART1 mode.
 *         This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
void UART1_ReceiverWakeUpCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* Enable the mute mode T_UART1 by setting the RWU bit in the CR2 register */
    T_UART1->CR2 |= UART1_CR2_RWU;
  }
  else
  {
    /* Disable the mute mode T_UART1 by clearing the RWU bit in the CR1 register */
    T_UART1->CR2 &= ((uint8_t)~UART1_CR2_RWU);
  }
}

/**
 * @brief  Returns the most recent received data by the T_UART1 peripheral.
 * @param  None
 * @retval The received data.
 */
uint8_t UART1_ReceiveData8(void)
{
  return ((uint8_t)T_UART1->DR);
}
...
uint16_t UART1_ReceiveData9(void)
{
  ...
  temp = (uint16_t)(((uint16_t)((uint16_t)T_UART1->CR1 & (uint16_t)UART1_CR1_R8)) << 1);
  return (uint16_t)((((uint16_t)T_UART1->DR) | temp) & ((uint16_t)0x01FF));
}

/**
 * @brief  Transmits 8 bit data through the T_UART1 peripheral.
...
void UART1_SendData9(uint16_t Data)
{
  ...
  T_UART1->CR1 &= ((uint8_t)~UART1_CR1_T8);
  /**< Write the transmit data bit [8]  */
  T_UART1->CR1 |= (uint8_t)(((uint8_t)(Data >> 2)) & UART1_CR1_T8);
  /**< Write the transmit data bit [0:7] */
  T_UART1->DR = (uint8_t)(Data);
}
...
{
  T_UART1->CR2 |= UART1_CR2_SBK;
}

/**
 * @brief  Sets the address of the T_UART1 node.
 * @param  UART1_Address: Indicates the address of the T_UART1 node.
 * @retval None
 */
void UART1_SetAddress(uint8_t UART1_Address)
{
  ...
  /* Clear the T_UART1 address */
  T_UART1->CR4 &= ((uint8_t)~UART1_CR4_ADD);
  /* Set the T_UART1 address node */
  T_UART1->CR4 |= UART1_Address;
}
...
{
  /* Set the T_UART1 guard time */
  T_UART1->GTR = UART1_GuardTime;
}
...
{
  /* Load the T_UART1 prescaler value*/
  T_UART1->PSCR = UART1_Prescaler;
}

/**
 * @brief  Checks whether the specified T_UART1 flag is set or not.
...
FlagStatus UART1_GetFlagStatus(UART1_Flag_TypeDef UART1_FLAG)
{
  ...
  /* Check the status of the specified T_UART1 flag*/
  if (UART1_FLAG == UART1_FLAG_LBDF)
  {
    if ((T_UART1->CR4 & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
    {
      /* UART1_FLAG is set*/
      status = SET;
    }
  ...
  {
    if ((T_UART1->CR2 & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
    {
      /* UART1_FLAG is set*/
      status = SET;
    }
  ...
  {
    if ((T_UART1->SR & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
    {
      /* UART1_FLAG is set*/
      status = SET;
    }
...
/**
 * @brief  Clears the T_UART1 flags.
...
void UART1_ClearFlag(UART1_Flag_TypeDef UART1_FLAG)
{
  ...
  {
    T_UART1->SR = (uint8_t) ~(UART1_SR_RXNE);
  }
  /* Clear the LIN Break Detection flag */
  else
  {
    T_UART1->CR4 &= (uint8_t) ~(UART1_CR4_LBDF);
  }
}

/**
 * @brief  Checks whether the specified T_UART1 interrupt has occurred or not.
 * @param  UART1_IT: Specifies the T_UART1 interrupt pending bit to check.
 *         This parameter can be one of the following values:
...
ITStatus UART1_GetITStatus(UART1_IT_TypeDef UART1_IT)
{
  ...
  /* Check the status of the specified T_UART1 pending bit*/
  if (UART1_IT == UART1_IT_PE)
  {
    /* Get the UART1_IT enable bit status*/
    enablestatus = (uint8_t)((uint8_t)T_UART1->CR1 & itmask2);
    /* Check the status of the specified T_UART1 interrupt*/

    if (((T_UART1->SR & itpos) != (uint8_t)0x00) && enablestatus)
    {
      /* Interrupt occurred*/
  ...
  {
    /* Get the UART1_IT enable bit status*/
    enablestatus = (uint8_t)((uint8_t)T_UART1->CR4 & itmask2);
    /* Check the status of the specified T_UART1 interrupt*/
    if (((T_UART1->CR4 & itpos) != (uint8_t)0x00) && enablestatus)
    {
      /* Interrupt occurred*/
      pendingbitstatus = SET;
    }
  ...
  {
    /* Get the UART1_IT enable bit status*/
    enablestatus = (uint8_t)((uint8_t)T_UART1->CR2 & itmask2);
    /* Check the status of the specified T_UART1 interrupt*/
    if (((T_UART1->SR & itpos) != (uint8_t)0x00) && enablestatus)
    {
      /* Interrupt occurred*/
...
/**
 * @brief  Clears the T_UART1 pending flags.
