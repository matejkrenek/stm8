 ...
 * @version V2.3.0
 * @date    16-June-2017
 * @brief   This file contains all functions prototype and macros for the T_SPI peripheral.
 ******************************************************************************
 * @attention
 *
...
/**
 * @brief  T_SPI data direction mode
...
/**
 * @brief  T_SPI Slave Select management
...
/**
 * @brief  T_SPI direction transmit/receive
 */
...
/**
 * @brief  T_SPI master/slave mode
 * Warning: element values correspond to MSTR bit position
 */
typedef enum
{
  SPI_MODE_MASTER = (uint8_t)0x04, /*!< T_SPI Master configuration */
  SPI_MODE_SLAVE = (uint8_t)0x00   /*!< T_SPI Slave configuration */
} SPI_Mode_TypeDef;

/**
 * @brief  T_SPI BaudRate Prescaler
 * Warning: element values correspond to BR bits position
 */
typedef enum
{
  SPI_BAUDRATEPRESCALER_2 = (uint8_t)0x00,   /*!< T_SPI frequency = frequency(CPU)/2 */
  SPI_BAUDRATEPRESCALER_4 = (uint8_t)0x08,   /*!< T_SPI frequency = frequency(CPU)/4 */
  SPI_BAUDRATEPRESCALER_8 = (uint8_t)0x10,   /*!< T_SPI frequency = frequency(CPU)/8 */
  SPI_BAUDRATEPRESCALER_16 = (uint8_t)0x18,  /*!< T_SPI frequency = frequency(CPU)/16 */
  SPI_BAUDRATEPRESCALER_32 = (uint8_t)0x20,  /*!< T_SPI frequency = frequency(CPU)/32 */
  SPI_BAUDRATEPRESCALER_64 = (uint8_t)0x28,  /*!< T_SPI frequency = frequency(CPU)/64 */
  SPI_BAUDRATEPRESCALER_128 = (uint8_t)0x30, /*!< T_SPI frequency = frequency(CPU)/128 */
  SPI_BAUDRATEPRESCALER_256 = (uint8_t)0x38  /*!< T_SPI frequency = frequency(CPU)/256 */
} SPI_BaudRatePrescaler_TypeDef;

/**
 * @brief  T_SPI Clock Polarity
...
/**
 * @brief  T_SPI Frame Format: MSB or LSB transmitted first
 * Warning: element values correspond to LSBFIRST bit position
 */
typedef enum
{
...
/**
 * @brief  T_SPI CRC Transmit/Receive
 */
typedef enum
{
...
/**
 * @brief  T_SPI flags definition - Warning : FLAG value = mapping position register
 */
typedef enum
{
...
{
  void (*deinit)();
  void (*init)(SPI_FirstBit_TypeDef, SPI_BaudRatePrescaler_TypeDef, SPI_Mode_TypeDef, SPI_ClockPolarity_TypeDef, SPI_ClockPhase_TypeDef, SPI_DataDirection_TypeDef, SPI_NSS_TypeDef, uint8_t);
  void (*enable)();
  void (*disable)();
  void (*sendData)(uint8_t);
  uint8_t (*receiveData)();
  FlagStatus (*getFlag)(SPI_Flag_TypeDef);
} SPI_Module;
...
/**
 * @brief  Macro used by the assert_param function in order to check the T_SPI Mode values
 */
