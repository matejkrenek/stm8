 ...
 * @version V2.3.0
 * @date    16-June-2017
 * @brief   This file contains all functions prototype and macros for the T_EXTI peripheral.
 ******************************************************************************
 * @attention
 *
...
/**
 * @brief  T_EXTI Sensitivity values for PORTA to PORTE
 */
typedef enum
{
  FALL_LOW = (uint8_t)0x00,  /*!< Interrupt on Falling edge and Low level */
  RISE_ONLY = (uint8_t)0x01, /*!< Interrupt on Rising edge only */
  FALL_ONLY = (uint8_t)0x02, /*!< Interrupt on Falling edge only */
  RISE_FALL = (uint8_t)0x03  /*!< Interrupt on Rising and Falling edges */
} EXTI_Sensitivity_TypeDef;

/**
 * @brief  T_EXTI Sensitivity values for TLI
 */
typedef enum
{
...
/**
 * @brief  T_EXTI PortNum possible values
 */
typedef enum
{
  EXTI_GPIOA = (uint8_t)0x00, /*!< GPIO Port A */
  EXTI_GPIOB = (uint8_t)0x01, /*!< GPIO Port B */
  EXTI_GPIOC = (uint8_t)0x02, /*!< GPIO Port C */
  EXTI_GPIOD = (uint8_t)0x03, /*!< GPIO Port D */
  EXTI_GPIOE = (uint8_t)0x04  /*!< GPIO Port E */
} EXTI_Port_TypeDef;
...
typedef struct
{
  void (*setSensitivity)(EXTI_Port_TypeDef, EXTI_Sensitivity_TypeDef);
} EXTI_Module;
...
#define IS_EXTI_SENSITIVITY_OK(SensitivityValue) \
  (((SensitivityValue) == FALL_LOW) ||           \
   ((SensitivityValue) == RISE_ONLY) ||          \
   ((SensitivityValue) == FALL_ONLY) ||          \
   ((SensitivityValue) == RISE_FALL))
...
/**
 * @brief  Macro used by the assert function in order to check the different values of the T_EXTI PinMask
 */
