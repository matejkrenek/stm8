 ...
 * @version V2.3.0
 * @date    16-June-2017
 * @brief  This file contains all functions prototype and macros for the T_I2C peripheral.
 ******************************************************************************
 * @attention
 *
...
/**
 * @brief  T_I2C duty cycle (fast mode only)
 */
typedef enum
{
...
/**
 * @brief  T_I2C Acknowledgement configuration
 */
typedef enum
{
...
/**
 * @brief  T_I2C Addressing Mode (slave mode only)
 */
typedef enum
{
...
/**
 * @brief  T_I2C transfer direction
...
typedef enum
{
  /*========================================
                       ...
                       T_I2C Master Events (Events grouped in order of communication)
                                                          ==========================================*/
  /**
   ...
   * released on the T_I2C bus (the bus is free, no other devices is communicating).
   *
   */
  /* --EV5 */
  ...
  /*========================================
                       ...
                       T_I2C Slave Events (Events grouped in order of communication)
                                                          ==========================================*/
   ...
   * the T_I2C peripheral detected a Start condition on the bus (generated by master
...
typedef struct
{
  void (*init)(uint32_t, uint16_t, I2C_DutyCycle_TypeDef, I2C_Ack_TypeDef, I2C_AddMode_TypeDef, uint8_t);
  void (*deinit)();
  void (*enable)();
  void (*disable)();
} I2C_Module;
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C duty cycles.
 */
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C addressing modes.
 */
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C interrupt types.
 */
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C communcation direction.
 */
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C flags.
 */
...
/**
 * @brief   Macro used by the assert function to check the T_I2C flags to clear.
 */

#define IS_I2C_CLEAR_FLAG_OK(FLAG) ((((uint16_t)(FLAG) & (uint16_t)0xFD00) == 0x00) && ((uint16_t)(FLAG) != 0x00))

/**
 * @brief   Macro used by the assert function to check the different T_I2C possible pending bits.
 */
...
/**
 * @brief  Macro used by the assert function to check the different T_I2C possible
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C possible events.
 */
...
/**
 * @brief   Macro used by the assert function to check the different T_I2C possible own address.
 */
...
/**
 * @brief   Macro used by the assert function to check that T_I2C Input clock frequency must be between 1MHz and 50MHz.
 */
#define IS_I2C_INPUT_CLOCK_FREQ_OK(FREQ) \
  (((FREQ) >= (uint8_t)1) && ((FREQ) <= I2C_MAX_INPUT_FREQ))

/**
 * @brief   Macro used by the assert function to check that T_I2C Output clock frequency must be between 1Hz and 400kHz.
 */
